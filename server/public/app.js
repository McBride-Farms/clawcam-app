...
function eventCard(e) {
  ...
  const lvl = Number.isInteger(e.vision_interest_level) ? e.vision_interest_level : null;
  const interestDot = lvl != null
    ? `<span class="interest interest-${lvl}" title="interest level ${lvl}/3">${lvl}</span>`
    : "";
  const actionPill = e.vision_suggested_action && e.vision_suggested_action !== "none"
    ? `<span class="action-pill action-${cssToken(e.vision_suggested_action)}">${esc(e.vision_suggested_action)}</span>`
    : "";
  const card = el(`
    <a href="#/event/${encodeURIComponent(e.event_id)}" class="event">
      ...
      <div class="meta">
        ...
        ${e.vision_caption ? `<div class="vision">${esc(e.vision_caption)}</div>` : ""}
      </div>
    </a>
  `);
  return card;
...
async function renderDetail(id) {
  ...
  try {
    ...
    if (event.vision_caption) {
      const lvl = Number.isInteger(event.vision_interest_level) ? event.vision_interest_level : null;
      const tagBits = [];
      if (lvl != null) tagBits.push(`<span class="interest interest-${lvl}" title="interest ${lvl}/3">${lvl}</span>`);
      if (event.vision_suggested_action && event.vision_suggested_action !== "none") {
        tagBits.push(`<span class="action-pill action-${cssToken(event.vision_suggested_action)}">${esc(event.vision_suggested_action)}</span>`);
      }
      body.appendChild(el(`
        <div class="vision-banner" title="Generated by the configured vision model on grunt-node2. Treat detail (colors, species) as approximate.">
          <div class="vision-head">
            <span class="vision-label">Vision</span>
            ${tagBits.join("")}
          </div>
          <span class="vision-text">${esc(event.vision_caption)}</span>
        </div>
      `));
    }
