...
function registeredDevice(host) {
  ...
  const byHost = stmts.getDevice.get(host);
  if (byHost) return byHost;
  return stmts.listDevices.all().find((d) => d.name === host) || null;
}
...
export function buildRouter() {
  ...
  r.get("/api/config", (req, res) => {
    ...
    res.json({
      hls_base: config.hlsBase || `${proto}://${hostname}:8888`,
      webrtc_base: config.webrtcBase || `${proto}://${hostname}:8889`,
      rtsp_base: config.rtspBase || `rtsp://${hostname}:8554`,
      webhook_url:
        config.webhookUrl ||
        `${proto}://${hostname}/hooks/clawcam`,
    });
  });
  ...
  r.patch("/api/devices/:host", requireAuth, express.json(), (req, res) => {
    ...
    if (!dev) return res.status(404).json({ error: "not_found" });
    const body = req.body || {};
    let system_prompt = dev.system_prompt;
    if (Object.prototype.hasOwnProperty.call(body, "system_prompt")) {
      const raw = body.system_prompt;
      system_prompt = typeof raw === "string" && raw.trim() ? raw.trim().slice(0, 4000) : null;
      stmts.setDeviceSystemPrompt.run({ host, system_prompt });
    }
    res.json({ device: { ...dev, system_prompt } });
  });
