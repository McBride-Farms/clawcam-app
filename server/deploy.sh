...
ssh "$REMOTE" "mkdir -p ${REMOTE_DIR_Q}/server ${REMOTE_DIR_Q}/data/media ~/.config/systemd/user"
rsync -az --delete \
...
ssh "$REMOTE" "install -m 0644 ${REMOTE_DIR_Q}/server/systemd/clawcam-app.service ~/.config/systemd/user/clawcam-app.service && systemctl --user daemon-reload && systemctl --user enable --now clawcam-app"

echo "==> enabling lingering so service runs after logout (best-effort; requires sudo)"
ssh "$REMOTE" "loginctl enable-linger ${REMOTE_USER} 2>/dev/null || echo 'lingering not enabled — run sudo loginctl enable-linger ${REMOTE_USER} later'"
